...
import { VERIFY_API_URL, PUBLIC_VERIFY_BASE } from "./api/client";
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-fog">
        <div className="w-8 h-8 rounded-full border-4 border-mint border-t-transparent animate-spin" />
      </div>
    );
  }
  if (!user) {
...
function normalizeData(apiData, fallback) {
  ...
  return {
    ...
    organizer: source.organizer || source.Organizer || source.organization || source.Organization || source.organiser || source.Organiser || "",
...
function ConfettiBurst({ burstKey }) {
  const pieces = useMemo(() => {
    if (!burstKey) return [];
    // Use colors from centralized color system for visual consistency
    const palette = [colors.mint, colors.amber, colors.danger, colors.ink, colors.steel, colors.fog];
...
function AppRoutes() {
  ...
  if (currentPath === "/payments") {
    return <RequireAuth><Payments /></RequireAuth>;
  }

  // Route: /issue-certificate
  if (currentPath === "/issue-certificate") {
    return <RequireAuth><IssueCertificate /></RequireAuth>;
  }
...
function CertificateVerifier() {
  ...
  async function buildCertificateImageDataUrl(data) {
    ...
    ctx.fillStyle = "#475569";
    ctx.font = "600 30px 'Times New Roman'";
    ctx.fillText(`Organized by ${data.organizer || "-"}`, canvas.width / 2, 624, 960);

    ctx.textAlign = "left";
    ctx.fillStyle = "#1e293b";
    ctx.font = "600 24px 'Segoe UI'";
    const certId = data.certificateId || data.rawId || "";
    const detailRows = [
      certId ? ["Certificate ID", certId] : null,
      hasValue(data.entryNumber) ? ["Entry Number", data.entryNumber] : null,
      ["Timestamp", data.timestamp],
    ].filter(Boolean);

    detailRows.forEach(([label, value], index) => {
      ctx.fillText(`${label}: ${value}`, 140, 760 + index * 46);
    });
    ...
    if (qrDataUrl) {
      ...
      if (qrImage) {
        ...
        ctx.fillRect(canvas.width - 400, 640, 250, 250);
        ctx.strokeStyle = "#d4a24c";
        ctx.lineWidth = 2;
        ctx.strokeRect(canvas.width - 400, 640, 250, 250);
        ctx.drawImage(qrImage, canvas.width - 385, 655, 220, 220);
      }
    }
    ...
    ctx.fillText("Scan to Verify", canvas.width - 275, 907);
    ctx.font = "700 22px 'Segoe UI'";
    ctx.fillText("Status: VERIFIED", canvas.width - 275, 935);
  ...
  async function verifyWithPayload(payload) {
    ...
    try {
      ...
      let data = await requestJson(VERIFY_API_URL);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-fog dark:bg-steel font-body text-ink dark:text-fog">
      ...
      <main className="relative mx-auto flex w-full max-w-5xl flex-col px-4 pb-12 pt-8 sm:px-8">
        <header className="mb-10 animate-rise rounded-3xl border border-steel/10 dark:border-steel/40 bg-white/70 dark:bg-white/10 p-6 shadow-soft backdrop-blur-md sm:p-8">
          <h1 className="font-display text-3xl font-bold leading-tight text-ink dark:text-white sm:text-4xl mb-3">
            Cerbro
          </h1>
          <p className="mb-4 text-2xl font-bold text-ink dark:text-white sm:text-3xl">
            Certificate Verified. Instantly.
          </p>
          <p className="mt-3 max-w-3xl text-sm text-steel/80 dark:text-fog/85 sm:text-base">
        ...
        <section className="grid gap-6">
          <form
            ...
            className="animate-rise rounded-3xl border border-steel/10 dark:border-steel/40 bg-white/80 dark:bg-white/10 p-6 shadow-soft backdrop-blur-sm [animation-delay:90ms] sm:p-8"
          >
            <p className="mb-2 block text-sm font-semibold text-steel dark:text-fog/90">Scan Your Certificate</p>
            ...
            <div className="mb-4 flex flex-wrap gap-3">
              ...
              <button
                ...
                className="rounded-xl border border-steel/20 dark:border-steel/40 bg-white dark:bg-white/10 px-5 py-2.5 text-sm font-semibold text-steel dark:text-fog/90 transition hover:bg-fog dark:hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-60"
              >
                Stop Scanner
              </button>
            </div>

            <div id={QR_REGION_ID} className="mb-4 overflow-hidden rounded-2xl border border-steel/15 dark:border-steel/40 bg-white dark:bg-white/10" />

            <label htmlFor="scan-text" className="mb-2 block text-sm font-semibold text-steel dark:text-fog/90">
            ...
            <textarea
              ...
              className="min-h-24 w-full rounded-xl border border-steel/20 dark:border-steel/40 bg-white dark:bg-white/10 px-4 py-3 text-sm text-ink dark:text-white outline-none transition focus:border-mint focus:ring-2 focus:ring-mint/30"
            />
        ...
        <section className="mt-6 animate-rise rounded-3xl border border-steel/10 dark:border-steel/30 bg-white/85 dark:bg-steel/20 p-6 shadow-soft backdrop-blur-sm [animation-delay:230ms] sm:p-8">
          <div className="mb-4 flex items-center justify-between gap-3">
            <h2 className="font-display text-2xl font-semibold text-ink dark:text-white">Verification Result</h2>
            <span
              className={`rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-wide ${
                result ? "bg-mint/20 text-mint dark:bg-mint/30 dark:text-white" : error ? "bg-danger/20 text-danger dark:bg-danger/30 dark:text-white" : "bg-amber/20 text-amber dark:bg-amber/30 dark:text-white"
              }`}
            >
              {statusText}
            </span>
          ...
          {result ? (
            <div className="result-ready-card mb-5 rounded-2xl border border-mint/20 dark:border-mint/35 bg-gradient-to-r from-mint/10 via-white to-amber/10 dark:bg-none dark:bg-steel/35 p-4">
              <p className="text-sm font-semibold text-ink dark:text-white">Certificate image ready</p>
              <p className="mt-1 text-sm text-steel/80 dark:text-fog/85">This certificate includes complete details and a verification QR.</p>
              <button
                type="button"
          ...
          {!result ? (
            <p className={`text-sm ${error ? "text-danger dark:text-red-300" : "text-steel/80 dark:text-fog/85"}`}>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {metaRows.map(([label, value], index) => (
                <article
                  key={label}
                  className="rounded-xl border border-steel/10 dark:border-steel/30 bg-fog/70 dark:bg-steel/30 p-4 text-sm [animation:rise_500ms_ease-out_both]"
                  style={{ animationDelay: `${index * 50}ms` }}
                >
                  <p className="text-xs uppercase tracking-wide text-steel/70 dark:text-fog/70">{label}</p>
                  <p className="mt-1 break-words font-semibold text-ink dark:text-white">{value}</p>
                </article>
              ))}

              {certificatePreviewUrl ? (
                <article className="sm:col-span-2 rounded-xl border border-steel/10 dark:border-steel/30 bg-white dark:bg-steel/30 p-4">
                  <p className="mb-3 text-xs uppercase tracking-wide text-steel/70 dark:text-fog/70">Digital Certificate Preview</p>
                  <img
                    src={certificatePreviewUrl}
                    alt="Digital certificate preview"
                    className="w-full rounded-lg border border-steel/10"
                  />
                  <p className="mt-3 text-sm text-steel/80 dark:text-fog/85">Formal certificate preview with embedded QR verification.</p>
                </article>
              ) : null}
            </div>
          )}
        ...
        <footer className="mt-12 text-center text-sm text-steel/70 dark:text-fog/75">
